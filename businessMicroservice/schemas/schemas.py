...
class EntityBase(BaseModel):
    ...
    phone: int
    address: str
    # EntityBase includes fields that are common to both creating and updating entities, excluding fields like id and created_at which are typically managed by the database.
class EntityCreate(EntityBase):
    # EntityCreate is used for input when creating a new entity. It inherits from EntityBase and includes only the fields necessary for creating an entity (excluding id and created_at).
    pass

class EntityUpdate(EntityBase):
    # EntityUpdate is used for input when updating an existing entity. It also inherits from EntityBase and excludes id and created_at.
    pass

class EntityRead(EntityBase):
    # EntityRead is used for output when reading an entity from the database. It includes all the fields, including id and created_at.
    id: int
    created_at: datetime
    class Config:
